...
extension SettingsStore {
        ...
        do {
            ...
            if self.launchAtStartupEnabled != enabled {
                let mismatchMessage = enabled
                    ? "macOS did not enable SpeachText1.0 in Login Items. Check System Settings > General > Login Items."
                    : "macOS still shows SpeachText1.0 in Login Items. Check System Settings > General > Login Items."
    ...
    private func launchAtStartupFailureMessage(for error: Error, enabling: Bool) -> String {
        ...
        {
            return "SpeachText1.0 could not \(action) launch at startup. This build may not be signed correctly for macOS Login Items."
        }

        if lowercasedDescription.contains("approval") ||
            lowercasedDescription.contains("authorize")
        {
            return "macOS needs approval before SpeachText1.0 can \(action) launch at startup. Check System Settings > General > Login Items."
        }

        return "SpeachText1.0 could not \(action) launch at startup. macOS reported: \(nsError.localizedDescription)"
    }
    ...
    private var compatibilityLoginItemName: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "SpeachText1.0"
    }
    ...
    private func runLaunchAtStartupAppleScript(_ source: String) throws {
        ...
        guard let script = NSAppleScript(source: source) else {
            throw NSError(
                domain: "SpeachText1.0LaunchAtStartup",
                code: 1,
...
private enum LaunchAtStartupSystemState {
    ...
    var message: String {
        switch self {
        case .enabled:
            return "SpeachText1.0 reflects the actual macOS login item state."
        case .disabled:
            return "SpeachText1.0 reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
        case .requiresApproval:
            return "macOS requires approval for SpeachText1.0 in Login Items before launch at startup becomes active."
        }
    }
}
#endif
